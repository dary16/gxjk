        ...
        <van-tabs v-model="active" swipeable background="#f0f0f0" @click="tabFn">
            <van-tab title="待处理" class="content">
                <van-pull-refresh v-model="unfinished.isDownLoading" @refresh="onDownRefresh('unfinished')">
                    <van-list id="unfinished-list" v-model="unfinished.isUpLoading" :finished="unfinished.upFinished" :immediate-check="false" :finished-text="unfinished.text" :offset="1" @load="onLoad('unfinished')">
                        <!-- 加载的内容-->
                        <van-loading type="spinner" v-if="unfinished.loading" />
                        <div v-if="unfinished.data.length > 0">
                            <van-row v-for="item in unfinished.data" :key="item.faultID" @click="infoIdFn(item)">
                                <van-col span="5"> <span class="list-icon"></span></van-col>
                                <van-col span="17">
                                    <van-cell :title="item.title" title-class="content-title" :value="item.status" value-class="btn-sendPolice"></van-cell>
            ...
            <van-tab title="已完成" class="content">
                <van-pull-refresh v-model="finished.isDownLoading" @refresh="onDownRefresh('finished')">
                    <van-list id="finished-list" v-model="finished.isUpLoading" :finished="finished.upFinished" :immediate-check="false" :finished-text="finished.text" :offset="1" @load="onLoad('finished')">
                        <!-- 加载的内容-->
                        <van-loading type="spinner" v-if="finished.loading" />
                        <div v-if="finished.data.length > 0">
                            <van-row v-for="item in finished.data" :key="item.faultID" @click="infoIdFn(item)">
                                <van-col span="5"> <span class="list-icon"></span></van-col>
                                <van-col span="17">
                                    <van-cell :title="item.title" title-class="content-title" :value="item.status" value-class="btn-isok"></van-cell>
                                    <p class="content-time">{{ item.time }}</p>
                                    <p class="content-info">【详情】 {{ item.content }}</p>
                                </van-col>
                                <van-col span="2">
...
export default {
    ...
    //监听属性 类似于data概念
    computed: {
        ...mapState(['activeIndex', 'getFoundFaultTime', 'newMsgNum'])
    },
    //监控data中的数据变化
    watch: {
    ...
    methods: {
        ...
        infoIdFn(val) {
            ...
            this.$router.push('/foundfaultMessage');
        },
        onDownRefresh(val) {
        ...
        getServerTime() {
            ...
            this._getInfo({
                ...
                callback: (res) => {
                    ...
                    div.innerHTML = res;
                    var listInfoData = div.querySelector('return').innerHTML;
                    setLoc('getFoundFaultTime', listInfoData);
                }
            });
        },
        tabFn(val) {
