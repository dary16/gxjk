...
<script>
  ...
  export default {
    ...
    //监听属性 类似于data概念
    computed: {
      ...mapState(['newMsgNum','handlingFaultNum','foundFaultNum'])
    },
    //监控data中的数据变化
    watch: {
      //运维
    ...
    methods: {
      ...
      setFn() {
        ...
        this.$router.push('/searchFault');
      },
      homeFn() {
        this.title = "故障通知列表";
        this.flagRe++;
        this.$router.push({ name: 'searchFaultList', params: { newMsgRow: this.newMsgRow } });
      ...
      getNewMsgLength() {
        const params = getPostData("findSensorFaultInfoNotViewNum", {
          ...
          "statuses": "",
          "clickTime": getLoc('getSearchFaultTime')
        });
        this._getInfo({
          ops: params,
          method: "post",
